        ...
        <div className="flex w-full flex-col gap-1">
          <div className="flex w-full items-center justify-between">
            {isEditing ? (
              <input
                ...
                className="w-24 border-b border-indigo-500 bg-transparent p-0 text-sm font-bold text-slate-700 outline-none"
              />
            ) : (
              <h3
                className="group/name flex cursor-pointer items-center gap-1.5 text-sm font-bold text-slate-800"
            ...
            <div className="flex items-center gap-1">
              {student.tags.map((t) => {
                ...
                return (
                  ...
                  >
                    <span className="cursor-help rounded bg-indigo-50 px-1 text-[9px] font-medium text-indigo-600 transition-colors hover:bg-indigo-100">
                      {t}
                    </span>
                  </div>
                );
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-baseline gap-1" title="学生综合能力值">
              <span className="text-[10px] font-medium text-slate-400">能力</span>
              <span className="text-xl leading-none font-black tracking-tight text-indigo-600">
            ...
            {!hideContestStatus && (
              <div className="flex max-w-[120px] flex-wrap justify-end gap-1">
                ...
                {isRecommended ? (
                  <span
                    className="rounded-sm border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 text-[9px] font-bold text-emerald-700"
                    title="已推荐"
                  >
                    已推荐
                  </span>
                  ...
                  (() => {
                    ...
                    return (
                      <span
                        className={`max-w-[80px] truncate rounded-sm border px-1.5 py-0.5 text-[9px] font-bold ${
                      ...
                      >
                        {student.lastAdvancementStatus === 'PASSED' ? '晋级' : '落榜'}{' '}
                        {displayContest}
                      </span>
                    );
                  })()
                ) : (
                  <span className="rounded-sm border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[9px] font-bold text-slate-500">
                    未参赛
                  </span>
                )}
              </div>

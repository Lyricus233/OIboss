...
export const RECRUITMENT_CONFIG = {
  ...
  ADVANCED: {
    label: '省队选手',
    cost: 100000,
    req: { reputation: 300, coachLevel: 4 },
...
请务必全程以 JSON 格式回复，每次对话你必须给出回答，不要输出任何普通文本。每次回复的格式必须为：
{
  ...
  "result": { // 仅在 is_finished 为 true 时包含此字段
    ...
    "reward": {
      ...
      {
        id: 'parent_recommendation',
        title: '家长推荐',
        openingMessage:
          '老师，我朋友家孩子也想学OI，我跟他推荐了你们机构，他可能这几天来咨询，麻烦你多关照一下！',
        systemPrompt: `你是一位非常满意机构教学的家长，主动给朋友推荐了这家机构。你希望老师能好好接待你的朋友，展示机构实力。如果玩家表示感谢并承诺会认真接待，你会很高兴。如果玩家态度敷衍或冷淡，你会觉得自己的推荐不被重视。
...
请务必全程以 JSON 格式回复，每次对话你必须给出回答，不要输出任何普通文本。每次回复的格式必须为：
{
  ...
  "is_finished": boolean, // 是否结束对话
  "result": { // 仅在 is_finished 为 true 时包含此字段
    "success": boolean, // 是否让家长满意
    "message": "结局描述",
    "reward": {
      "potentialStudents": number, // 成功 +15~+25（口碑传播），失败 +5
      "reputation": number, // 成功 +8~+12, 失败 +2
      "studentSatisfaction": number, // 成功 +10（现有家长更满意），失败 -3
      "bossStress": number // 成功 -8, 失败 +5
    }
  }
}`,
      },
